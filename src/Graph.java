...
public class Graph {
    ...
    void deleteEdge (Node origin, Node destination) {
        edges.removeIf(edge -> edge.start == origin && edge.end == destination);
    }
    ...
    void createMatrix () {
        Hashtable<Node,Integer> row = new Hashtable<>();
        /*
    ...
    void matrixAdjacent () {
        ...
        */
        createMatrix();
        edges.stream().filter( edge -> adjacencyMatrix.get(edge.start).containsKey(edge.end) )
                .forEach( edge -> adjacencyMatrix.get(edge.start).put(edge.end, 1) );
    }
    ...
    void listAdjacent () {
        ...
        edges.forEach(edge -> adding(edge.start, edge.end));
        ...
        edges.stream().filter(e -> !adjacencyList.containsKey(e.end)).
                forEach(e -> adjacencyList.put(e.end, new ArrayList<>()));
    }
